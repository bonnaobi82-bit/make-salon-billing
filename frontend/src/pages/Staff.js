          ...
          {loading ? (
            ...
            staff.map((member) => (
              <div key={member.id} className="stat-card" data-testid={`staff-card-${member.id}`}>
                ...
                <div className="mt-4 pt-4 border-t border-[#E8EAE6]">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-[#6B726C]">Commission</span>
                    <span className="font-medium text-[#1B3B36]">{member.commission_rate}%</span>
                  </div>
                </div>
              </div>
            ))
